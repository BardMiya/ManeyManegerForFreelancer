...
<script setup>
    ...
    const servicerListForCellComputed = computed({
        get: () => servicerList.map(item => ({name: item.name, id:item.personal_cd})),
    ...
    const getTransaction = async (account) => {
        ...
        const params = {
            account_CD: account,
            dateFrom: periodBeginComputed.value,
            dateTo: periodEndComputed.value
        }
     ...
     */
    const setBalance = (i) => {
        jspreadsheetObj.setValue(`H${i+1}`,`=IF(B${i+1},"",SUM(F1:F${i+1})-SUM(G1:G${i+1}))`,false)
    }
    /**
     * ヘッダ用行をセット
    ...
    onMounted( async () => {
        ...
        // const accounts = await useGetAccount({parameter: {type:[3,4,5,6]}})
        const accounts = await accountGetter.get({parameter: {type:[3,4,5,6]}})
        ledgerListComputed.value = accounts
        // const resServicer = await useGetServicer({})
